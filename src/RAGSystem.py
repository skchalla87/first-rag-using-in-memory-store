...
class RAGSystem:
    ...
    def retrieve(
        ...
        with self.tracer.start_as_current_span("rag-retrieve") as span:
            ...
            span.set_attribute("top_sources", str([meta["source"] for _, _, meta in top_results[:3]]))
        return top_results
    ...
    def query(self, question: str, top_k: int = 3, verbose: bool = True) -> str:
        ...
        Answer:"""
            ...
            span.set_attribute("num_chunks", len(retrieved_docs))
            span.set_attribute("retrieval_ms", retrieval_ms)
            span.set_attribute("generation_ms", generation_ms)
            span.set_attribute("refused", "don't have information" in answer.lower())
    ...
    def _log_query(
        self, query_id, question, answer, retrieved_docs, retrieval_ms, generation_ms
    ):
        log_entry = {
            "query_id": query_id,
            ...
            "top_sources": [meta["source"] for _, _, meta in retrieved_docs[:3]],
            "top_scores": [round(score, 3) for _, score, _ in retrieved_docs[:3]],
            "refused": "don't have information" in answer.lower(),
        }
